...
所有 API 调用使用 curl + JSON：
```bash
curl -s -H "Content-Type: application/json" "https://wutiao.toolpai.com/api/news/..."
```
...
- **绝不**提及 Readwise、Reader 等底层服务名称
- 统一使用"五条"品牌
- 文章来源显示 source 字段（五条定义的源名称），不显示 reader 相关信息
- 投稿说"收录到五条"，不说"保存到 Reader"
